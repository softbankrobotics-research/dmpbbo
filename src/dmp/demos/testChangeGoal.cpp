...
int main(int n_args, char** args)
{
  ...
  if (n_args!=3) 
  {
    cerr << "Usage: " << args[0] << "<directory>" << "<trajFile>" << endl;
    return -1;
  }
  save_directory = string(args[1]);
  traj_file = string(args[2]);

  
  cout << "Reading trajectory from TXT file: " << traj_file << endl;
  Trajectory trajectory = Trajectory::readFromFile(traj_file);
