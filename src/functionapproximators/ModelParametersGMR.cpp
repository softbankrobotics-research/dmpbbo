  ...
  covars_y_x_(covars_y_x)
{
  ...
  //all_values_vector_size_ = 0;
  UnifiedModel* unifiedModel = toUnifiedModel();
  all_values_vector_size_ = unifiedModel->initializeAllValuesVectorSize();
  n_observations_ = 0;
...
{
  UnifiedModel* unifiedModel = toUnifiedModel();
  unifiedModel->getParameterVectorMask(selected_values_labels, selected_mask);
...
void ModelParametersGMR::getParameterVectorAll(VectorXd& values) const
{
  ...
  UnifiedModel* unifiedModel = toUnifiedModel();
  unifiedModel->getParameterVectorAll(values);
...
{  
  UnifiedModel* unifiedModel = toUnifiedModel();
  unifiedModel->setParameterVectorAll(values);
  //int offset = 0;
  //for (size_t i = 0; i < centers_.size(); i++)
  //{
